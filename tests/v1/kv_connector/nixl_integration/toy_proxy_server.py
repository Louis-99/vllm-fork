...
async def lifespan(app: FastAPI):
    ...
    for i, (host, port) in enumerate(global_args.prefiller_instances):
        ...
        app.state.prefill_clients.append({
            'client':
            httpx.AsyncClient(timeout=None, base_url=prefiller_base_url, limits=httpx.Limits(max_keepalive_connections=512, max_connections=512)),
            'host':
            host,
            'port':
            port,
    ...
    for i, (host, port) in enumerate(global_args.decoder_instances):
        ...
        app.state.decode_clients.append({
            'client':
            httpx.AsyncClient(timeout=None, base_url=decoder_base_url, limits=httpx.Limits(max_keepalive_connections=512, max_connections=512)),
            'host':
            host,
            'port':
            port,
