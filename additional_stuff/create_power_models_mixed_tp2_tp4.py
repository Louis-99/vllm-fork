#!/usr/bin/env python3
"""
Unified power model for prefill and decode with separate feature columns (TP2 and TP4 only).
...
- Filters out TP8 data to create separate TP2+TP4 model
...
def get_param_grid():
    ...
    param_grid = {
        ...
        'est__num_leaves': [60],
        'est__num_iterations': [300],
        'est__learning_rate': [0.08],
        'est__min_child_samples': [40],
...
def main():
    parser = argparse.ArgumentParser(description='Train unified power model for prefill and decode (TP2+TP4 only)')
    parser.add_argument('--model-dir', default=('/export2/obasit/ClusterLevelServing/vllm_profiler_logs/power_model/tp2_tp4'), help='Directory to store trained models')
    ...
    UNIFIED_OUT = 'unified_power_model_tp2_tp4.txt'
    ...
    # Save combined data
    df_combined.to_csv(os.path.join(MODEL_DIR, 'unified_power_data_cleaned_tp2_tp4.csv'), index=False)
    
    # Train unified model
    print('\n=== Training unified power model (TP2+TP4) ===')
    ...
    if not os.path.exists(model_path):
        ...
        for label, sample in samples.items():
            ...
            try:
                ...
                print(f'  {label}: {pred:.2f}W  ({(time.time()-t0)*1e3:.3f}ms)')
